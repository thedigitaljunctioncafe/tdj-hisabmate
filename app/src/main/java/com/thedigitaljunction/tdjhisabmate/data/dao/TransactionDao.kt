...
interface TransactionDao {
    ...
    @Query("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'EXPENSE' AND dateMillis >= :startMillis AND dateMillis <= :endMillis")
    fun getSumExpenseBetween(startMillis: Long, endMillis: Long): Flow<Long>

    @Query("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'INCOME' AND dateMillis >= :startMillis AND dateMillis <= :endMillis")
    fun getSumIncomeBetween(startMillis: Long, endMillis: Long): Flow<Long>

    @Query("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'EXPENSE' AND categoryId = :categoryId AND dateMillis >= :startMillis AND dateMillis <= :endMillis")
    fun getSumCategoryExpenseBetween(categoryId: Long, startMillis: Long, endMillis: Long): Flow<Long>
